...
import com.startupsreactor.telegram.config.KafkaSseConsumer;
import com.startupsreactor.telegram.config.KafkaSseProducer;
...
class TelegramKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/telegram-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/telegram-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/telegram-kafka/unregister"));
                return;
            }
        }
